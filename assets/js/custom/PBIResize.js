...
function embedCustomLayoutReport() {
    ...
    $(window).resize(function () {
        ...
        if (width < 576) {
            ...
            $('#embedContainer').css("height: 2229px");
        }
        else if (width < 768) {
            LayoutShowcaseState.columns = ColumnsNumber.Two;
            $('#embedContainer').css("height: 1650px");
        }
        else {
            LayoutShowcaseState.columns = ColumnsNumber.Three;
            $('#embedContainer').css("height: 670px");
        }
    });
    ...
    LoadLayoutShowcaseReportIntoSession().then(function () {
        ...
        var accessToken = raccessToken;

        // Get embed URL from session
        var embedUrl = rembedUrl;

        // Get report Id from session
        var embedReportId = rembedReportId;
...
function embedCampaignLayoutReport() {
    // Defualt columns value is three columns
    //$(window).resize(function () {
    //    var width = $(document).width();
    //    if (width < 576) {
    ...
    LoadLayoutShowcaseReportIntoSession().then(function () {
        ...
        // Get embed application token from session
        var accessToken1 = raccessToken1;
        var accessToken2 = raccessToken1;
        var accessToken3 = raccessToken1;
        // Get embed URL from session
        var embedUrl1 = rembedUrl1;
        var embedUrl2 = rembedUrl2;
        var embedUrl3 = rembedUrl3;
        // Get report Id from session
        var embedReportId1 = rembedReportId1;
        var embedReportId2 = rembedReportId2;
        var embedReportId3 = rembedReportId3;
        ...
        // Get a reference to the embedded report HTML element
        var campaignReport1 = $('#campaignReport1')[0];
        var campaignReport2 = $('#campaignReport2')[0];
        var campaignReport3 = $('#campaignReport3')[0];
        // Embed the report and display it within the div container
        LayoutShowcaseState.layoutReport = powerbi.embed(campaignReport1, config1);
        LayoutShowcaseState.layoutReport = powerbi.embed(campaignReport2, config2);
        LayoutShowcaseState.layoutReport = powerbi.embed(campaignReport3, config3);
        // Report.on will add an event handler for report loaded event
        LayoutShowcaseState.layoutReport.on("loaded", function () {
